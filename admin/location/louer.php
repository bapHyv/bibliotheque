...
$sqlEtatLivre = "SELECT * FROM etat_livre 
...
<?php
$styleSheet =
    '<link rel="stylesheet" href="' . URL_ADMIN . 'css/liste-livre.css">';
...
<?php if (isset($_SESSION['error_livre']) && $_SESSION['error_livre'] == true) {
    ...
    <a href="<?= URL_ADMIN ?>location/index.php" class="btn btn-info">Retour</a>
                    <div class="mt-5">
                        <div style="display: flex;">
                            <img style="height: 600px;" src="<?= URL_ADMIN ?>/img/livre/<?= $livre[
    'illustration'
] ?>" alt="">
                            <div class="ml-5 mr-5">
                                <p class="resume"><b>Résumé: </b><?= $livre[
                                    'resume'
                                ] ?></p>
                                <p><b>Prix:</b> <?= $livre['prix'] ?></p>
                                <p><b>Nombre de pages:</b> <?= $livre[
                                    'nb_pages'
                                ] ?></p>
                                <p><b>Date d'achat:</b> <?= $livre[
                                    'date_achat'
                                ] ?></p>
                            </div>
                        </div>
...
<form action="<?= URL_ADMIN ?>location/action.php" method="POST" enctype='multipart/form-data'>
    ...
    <input type="hidden" value="<?= $livre[
        'id'
    ] ?>" name="id" id="id-utilsateur">
    ...
    <select name="usager" id="usager">
        ...
        <?php foreach ($usagers as $usager): ?>
            <option value="<?= $usager['id'] ?>"><?= $usager['nom'] .
    ' ' .
    $usager['prenom'] ?></option>
        <?php endforeach; ?>
    </select>
    <br>
    <label for="etat">État:</label>
    <select name="etat" id="etat">
        <option value="<?= $etatLivre['id_etat'] ?>"><?= $etatLivre[
    'libelle'
] ?></option>
        <?php foreach ($etats as $etat): ?>
            <?php if ($etat['libelle'] != $etatLivre['libelle']): ?>
              <option value="<?= $etat['id'] ?>"><?= $etat[
    'libelle'
] ?></option>
            <?php endif; ?>
          <?php endforeach; ?>
